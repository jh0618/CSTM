...
CanineFilter::TauLPF::TauLPF(double DT, double cutoffFreq)
    : mbIsFirstRun(true)
    , mDT(DT)
    , mCutoffFreq(cutoffFreq)
    , mAlpha(2 * 3.141592 * mCutoffFreq * mDT / (2 * 3.141592 * mCutoffFreq * mDT + 1))
{
    ...
    {
        mInputData[idx] = 0.0;
        mPreviousData[idx] = 0.0;
        mFilteredData[idx] = 0.0;
    }
}

double* CanineFilter::TauLPF::GetFilteredVar(double* data)
{
    for (int idx = 0; idx < 16; idx++)
    {
        mInputData[idx] = data[idx];
...
void CanineFilter::TauLPF::doFiltering()
{
    if (mbIsFirstRun == true)
    {
        for (int idx = 0; idx < 16; idx++)
