#include <canine_hardware/CanMotorFL.hpp>
...
{
    switch (sharedMemory->canState[FL_IDX])
    {
    case CAN_NO_ACT:
    {
        usleep(10);
    ...
    case CAN_MOTOR_ON:
    {
        ...
        turnOnMotor();
        sharedMemory->canState[FL_IDX] = CAN_READ_ERROR;
        break;
    }
    case CAN_INIT:
    {
        canInit();
        sharedMemory->canState[FL_IDX] = CAN_MOTOR_ON;
        break;
    }
    case CAN_MOTOR_OFF:
    {
        ...
        sharedMemory->canState[FL_IDX] = CAN_READ_ENCODER;
        break;
    }
    case CAN_SET_TORQUE:
    {
...
void CanMotorFL::mapReadValue()
{
    for (int motorIndex = 0; motorIndex < MOTOR_NUM_LEG_PER_CAN; motorIndex++)
    {
        ...
        switch (command)
        {
            ...
            {
//                std::cout<<"[CAN-FL] Turned on motor: "<<motorIndex<<std::endl;
            }
                break;
            case CAN_CMD_TURN_OFF:
            {
//                std::cout<<"[CAN-FL] Turned off motor: "<<motorIndex<<std::endl;
            }
                break;
            ...
            default:
            {
                std::cout<<"[CAN-FL] Command is not matched. Motor idx: "<<motorIndex<<"\tCommand : "<<command<<std::endl;
...
void CanMotorFL::checkConnection()
{
    ...
    if (mDisconnectedCount == 10)
    {
        ...
        std::cerr<<"[CAN-FL] CAN Communication error."<<std::endl;
    }
}
...
void CanMotorFL::setLoopGain(int angleKp, int angleKi, int speedKp, int speedKi, int currentKp, int currentKi)
{
    u_int8_t data[3][8];
...
void CanMotorFL::setEncoderZeroPosition(int motorIdx)
{
    u_int8_t data[3][8];
