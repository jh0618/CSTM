#include <canine_hardware/CanMotorHL.hpp>
#include "camel-tools/ThreadGenerator.hpp"

CanMotorHL::CanMotorHL(std::string canName)
    : mCanName(canName)
{
    ...
    mGearRatio = 9;
    torque2int[HLHR_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = 24.0385;
    torque2int[HLHP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = 24.0385;
    torque2int[HLKP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = 24.0385;

    mMotorId[HLHR_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = MOTOR_HLHR_ID;
    mMotorId[HLHP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = MOTOR_HLHP_ID;
    mMotorId[HLKP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = MOTOR_HLKP_ID;

    mAxis[HLHR_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = -1.0;
    mAxis[HLHP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = 1.0;
    mAxis[HLKP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = 1.0;

    mAngularPositionOffset[HLHR_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = HLHR_POS_OFFSET;
    mAngularPositionOffset[HLHP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = HLHP_POS_OFFSET;
    mAngularPositionOffset[HLKP_IDX - HL_IDX * MOTOR_NUM_LEG_PER_CAN] = HLKP_POS_OFFSET;
...
void CanMotorHL::CanFunction()
{
    switch (sharedMemory->canState[HL_IDX])
    {
    case CAN_NO_ACT:
    {
        usleep(10);
    ...
    case CAN_MOTOR_ON:
    {
        ...
        turnOnMotor();
        sharedMemory->canState[HL_IDX] = CAN_READ_ERROR;
        break;
    }
    case CAN_INIT:
    {
        canInit();
        sharedMemory->canState[HL_IDX] = CAN_MOTOR_ON;
        break;
    }
    case CAN_MOTOR_OFF:
    {
        ...
        sharedMemory->canState[HL_IDX] = CAN_READ_ENCODER;
        break;
    }
    case CAN_SET_TORQUE:
    {
...
void CanMotorHL::canSend(int motorIndex, const u_int8_t* data)
{
    if(motorIndex == 0)
    {
        for(int i = 0 ; i < 3 ; i++)
...
void CanMotorHL::mapReadValue()
{
    for (int motorIndex = 0; motorIndex < MOTOR_NUM_LEG_PER_CAN; motorIndex++)
    {
        ...
        switch (command)
        {
            ...
            {
//                std::cout<<"[CAN-HL] Turned on motor: "<<motorIndex<<std::endl;
            }
                break;
            case CAN_CMD_TURN_OFF:
            {
//                std::cout<<"[CAN-HL] Turned off motor: "<<motorIndex<<std::endl;
            }
                break;
            ...
            default:
            {
                std::cout<<"[CAN-HL] Command is not matched. Motor idx: "<<motorIndex<<"\tCommand : "<<command<<std::endl;
...
void CanMotorHL::checkConnection()
{
    ...
    if (mDisconnectedCount == 10)
    {
        ...
        std::cerr<<"[CAN-HL] CAN Communication error."<<std::endl;
    }
}

void CanMotorHL::readEncoder()
{
...
void CanMotorHL::readMotorErrorStatus()
{
...
void CanMotorHL::setLoopGain(int angleKp, int angleKi, int speedKp, int speedKi, int currentKp, int currentKi)
{
    u_int8_t data[3][8];
...
void CanMotorHL::setEncoderZeroPosition(int motorIdx)
{
    u_int8_t data[3][8];
