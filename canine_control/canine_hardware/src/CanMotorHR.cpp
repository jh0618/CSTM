#include <canine_hardware/CanMotorHR.hpp>
#include "camel-tools/ThreadGenerator.hpp"

CanMotorHR::CanMotorHR(std::string canName)
    : mCanName(canName)
{
    ...
    mGearRatio = 9;
    torque2int[HRHR_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = 24.0385;
    torque2int[HRHP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = 24.0385;
    torque2int[HRKP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = 24.0385;

    mMotorId[HRHR_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = MOTOR_HRHR_ID;
    mMotorId[HRHP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = MOTOR_HRHP_ID;
    mMotorId[HRKP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = MOTOR_HRKP_ID;

    mAxis[HRHR_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = -1.0;
    mAxis[HRHP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = -1.0;
    mAxis[HRKP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = -1.0;

    mAngularPositionOffset[HRHR_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = HRHR_POS_OFFSET;
    mAngularPositionOffset[HRHP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = HRHP_POS_OFFSET;
    mAngularPositionOffset[HRKP_IDX - HR_IDX * MOTOR_NUM_LEG_PER_CAN] = HRKP_POS_OFFSET;
...
void CanMotorHR::CanFunction()
{
    switch (sharedMemory->canState[HR_IDX])
    {
    case CAN_NO_ACT:
    {
        usleep(10);
    ...
    case CAN_MOTOR_ON:
    {
        ...
        turnOnMotor();
        sharedMemory->canState[HR_IDX] = CAN_READ_ERROR;
        break;
    }
    case CAN_INIT:
    {
        canInit();
        sharedMemory->canState[HR_IDX] = CAN_MOTOR_ON;
        break;
    }
    case CAN_MOTOR_OFF:
    {
        ...
        sharedMemory->canState[HR_IDX] = CAN_READ_ENCODER;
        break;
    }
    case CAN_SET_TORQUE:
    {
...
void CanMotorHR::canSend(int motorIndex, const u_int8_t* data)
{
    if(motorIndex == 0)
    {
        for(int i = 0 ; i < 3 ; i++)
...
void CanMotorHR::mapReadValue()
{
    for (int motorIndex = 0; motorIndex < MOTOR_NUM_LEG_PER_CAN; motorIndex++)
    {
        ...
        switch (command)
        {
            ...
            {
//                std::cout<<"[CAN-HR] Turned on motor: "<<motorIndex<<std::endl;
            }
                break;
            case CAN_CMD_TURN_OFF:
            {
//                std::cout<<"[CAN-HR] Turned off motor: "<<motorIndex<<std::endl;
            }
                break;
            ...
            default:
            {
                std::cout<<"[CAN-HR] Command is not matched. Motor idx: "<<motorIndex<<"\tCommand : "<<command<<std::endl;
...
void CanMotorHR::checkConnection()
{
    ...
    if (mDisconnectedCount == 10)
    {
        ...
        std::cerr<<"[CAN-HR] CAN Communication error."<<std::endl;
    }
}

void CanMotorHR::readEncoder()
{
...
void CanMotorHR::readMotorErrorStatus()
{
...
void CanMotorHR::setLoopGain(int angleKp, int angleKi, int speedKp, int speedKi, int currentKp, int currentKi)
{
    u_int8_t data[3][8];
...
void CanMotorHR::setEncoderZeroPosition(int motorIdx)
{
    u_int8_t data[3][8];
